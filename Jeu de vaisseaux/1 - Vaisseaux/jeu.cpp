  ...
  do // Boucle principale
  {
   ...
   if(vHyperEspace == false)
   {
    ...
    if(kbhit())
    {
      ...
      if(vUser == 45) // - moins de vie
      	cJoueur.vie -= 20;
      if(vUser == 43) // r + plus de vie
      	cJoueur.vie += 20;
      if(vUser == 113) // q - vague suivante
      {
         vArrive = false;
         ...
         bar(0,0,799,768);
		   for(int i = 0; i < 11; i++)
   			for(int j = 0; j < 5; j++)
	         {
     		   	cVague.formation[i][j].type = 0;
        			cVague.formation[i][j].Effacer(vCountMain);
  		      }
      	cVague.num++;
         cVague.Positionner(300);
      }
...
void Gagner(char vNom[])
{
   ...
   strcat(vText,vNom);
   strcat(vText,"@");
   strcat(vText,"Vous avez sauvé la galaxie  etc..."); // @ pour sauter à la ligne
   AfficherText(vText,&vX,&vY);
