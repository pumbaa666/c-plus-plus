...
int TMaFenetre::GetFile(char *fileName)
{
    ...
    OpenFile.lpstrFilter =
        "Text Files(*.txt;*.doc)\0*.txt;*.doc\0All Files (*.*)\0*.*\0";
...
void TMaFenetre::CmFichierRelire()
{
   ...
   TFileOpenDialog::TData FilenameData (OFN_FILEMUSTEXIST | OFN_HIDEREADONLY | OFN_PATHMUSTEXIST, "All Files (*.*)|*.*|Text Files (*.txt)|*.txt|", 0, "", "*");
   if(TFileOpenDialog(this, FilenameData).Execute() == IDOK)
   {
  		GetFile(vNomFichier);
	   DataFile.open(vNomFichier, ios::in);
	   while(!feof(f))
   	{
	   	tTemp[0] = '\0';
   		tCat[0] = '\0';
