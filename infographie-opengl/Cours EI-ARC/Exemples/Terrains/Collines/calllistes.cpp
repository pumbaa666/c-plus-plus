    ...
    glTranslatef(-LARGEUR_TERRAIN/2, 0, -LARGEUR_TERRAIN/2);
     for(xx = 0; xx  < LARGEUR_TERRAIN-1; xx += nStep)
     {
       glBegin(GL_TRIANGLE_STRIP);
        for(zz = 0; zz  < LARGEUR_TERRAIN; zz += nStep)
        {
          glColor3f(0.0f, hauteur[xx][zz], 0.0f);
          if(hauteur[xx][zz] > altitudeNeige) // Blanc
            glColor3f(1.0f, 1.0f, 1.0f);
          if(hauteur[xx][zz] <= 0.0f) // Bleu
            glColor3f(0.0f, 0.0f, 1.0f);
          glVertex3f((float)xx, hauteur[xx][zz], (float)zz);

          glColor3f(0.0f, hauteur[xx+nStep][zz], 0.0f);
          if(hauteur[xx+nStep][zz] > altitudeNeige) // Blanc
            glColor3f(1.0f, 1.0f, 1.0f);
          if(hauteur[xx+nStep][zz] <= 0.0f) // Bleu
            glColor3f(0.0f, 0.0f, 1.0f);
          glVertex3f((float)xx+nStep, hauteur[xx+nStep][zz], (float)zz);
        }
       glEnd();
     }
     glPopMatrix();
