...
{
	// Spin object
	if (keys[VK_LEFT])	yspeed -= 0.01f;	        // 'Arrow Left' decrease yspeed
	if (keys[VK_RIGHT])	yspeed += 0.01f;		// 'Arrow Right' increase yspeed
	if (keys[VK_UP])	xspeed -= 0.01f;		// 'Arrow Up' decrease xspeed
	if (keys[VK_DOWN])	xspeed += 0.01f;		// 'Arrow Down' increase xspeed
