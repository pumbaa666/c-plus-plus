...
int main(int argc,char *argv[])
{
	...
	new FenetreVisualisation(&application);
...
FXDEFMAP(FenetreVisualisation) FenetreVisualisationMap[]={
	
	FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_ABOUT,FenetreVisualisation::onCmdAbout),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_SURF_NURBS,FenetreVisualisation::onCreerSurfNURBS),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_SURF_BEZIERRAT,FenetreVisualisation::onCreerSurfBezierRat),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_SURF_BEZIER,FenetreVisualisation::onCreerSurfBezier),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_SURF_BSPLINE,FenetreVisualisation::onCreerSurfBSpline),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_SURF_POLYEDRIQUE,FenetreVisualisation::onCreerSurfacePolyedrique),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_LIGNE,FenetreVisualisation::onCreerLigne),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_SEGMENT,FenetreVisualisation::onCreerSegment),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_POLYGONE,FenetreVisualisation::onCreerPolygone),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_OPEN,FenetreVisualisation::onCmdOpen),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_NEWVIEWER,FenetreVisualisation::onCmdNewViewer),
		FXMAPFUNC(SEL_UPDATE,FenetreVisualisation::ID_QUERY_MODE,FenetreVisualisation::onUpdMode),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_FEN_PROP_NURBS, FenetreVisualisation::onCmdPropNurbs),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_SUPPRIMER_OBJET3D, FenetreVisualisation::onSupprimerObjet3D),
		FXMAPFUNC(SEL_COMMAND,FXWindow::ID_QUERY_MENU,FenetreVisualisation::onQueryMenu),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_ENREGISTRER, FenetreVisualisation::onEnregistrer),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_ENREGISTRER_SOUS, FenetreVisualisation::onEnregistrerSous),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_NOUVEAU, FenetreVisualisation::onNouveau),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_COURBE_NURBS,FenetreVisualisation::onCreerCourbeNURBS),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_COURBE_BEZIERRAT,FenetreVisualisation::onCreerCourbeBezierRat),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_COURBE_BEZIER,FenetreVisualisation::onCreerCourbeBezier),
		FXMAPFUNC(SEL_COMMAND,FenetreVisualisation::ID_CREER_COURBE_BSPLINE,FenetreVisualisation::onCreerCourbeBSpline),

};


FXIMPLEMENT(FenetreVisualisation,FXMainWindow,FenetreVisualisationMap,ARRAYNUMBER(FenetreVisualisationMap))
...
FenetreVisualisation::FenetreVisualisation(FXApp* a):FXMainWindow(a,"Courbes et Surfaces",NULL,NULL,DECOR_ALL,0,0,800,600){
	...
	toolbar=new FXToolbar(this,LAYOUT_SIDE_TOP|LAYOUT_FILL_X,0,0,0,0, 4,4,0,0, 0,0);
	...
	FXVerticalFrame *box=new FXVerticalFrame(frame,FRAME_SUNKEN|FRAME_THICK|LAYOUT_FILL_X|LAYOUT_FILL_Y|LAYOUT_TOP|LAYOUT_LEFT,0,0,0,0, 0,0,0,0);
	...
	//glvisual=new FXGLVisual(getApp(),0);
	
	// Make it active
	mdiclient->setActiveChild(mdichild);
	
	// Drawing gl canvas
	viewer=new FXGLViewer(mdichild,glvisual,this,ID_GLVIEWER,LAYOUT_FILL_X|LAYOUT_FILL_Y|LAYOUT_TOP|LAYOUT_LEFT);
	
	// Tab book with switchable panels
	FXVerticalFrame *frame2=new FXVerticalFrame(frame,LAYOUT_TOP|LAYOUT_LEFT,0,0,0,0, 0,0,0,0);
	
	FXTabBook* panels=new FXTabBook(frame2);
	new FXButton(frame2,"&Propriétés\tModifie les paramètres d'un objet 3D",NULL,this,ID_FEN_PROP_NURBS,FRAME_THICK|FRAME_RAISED|LAYOUT_TOP|LAYOUT_LEFT);	
	new FXButton(frame2,"&Supprimer\tSupprime un objet 3D",NULL,this,ID_SUPPRIMER_OBJET3D,FRAME_THICK|FRAME_RAISED|LAYOUT_TOP|LAYOUT_LEFT);	
	
	new FXTabItem(panels,"Angles\tAngles de la caméra\tCliquer sur l'onglet Angles.");
	...
	x_dial->setTipText("Rotation sur X");
	...
	y_dial->setTipText("Rotation sur Y");
	...
	z_dial->setTipText("Rotation sur Z");
	...
	fov->setTipText("Champs de visualisation");
	...
	// Les couleurs
	new FXTabItem(panels,"Couleurs\tCouleurs\tCliquer sur l'onglet Couleurs.");
	...
	new FXLabel(colors,"Fond :",NULL,LAYOUT_RIGHT|LAYOUT_CENTER_Y|JUSTIFY_RIGHT);
	new FXColorWell(colors,0,mdiclient,FXGLViewer::ID_BACK_COLOR,COLORWELL_OPAQUEONLY|LAYOUT_TOP|LAYOUT_LEFT, 0,0,0,0 ,0,0,0,0);
	new FXLabel(colors,"Objets :",NULL,LAYOUT_RIGHT|LAYOUT_CENTER_Y|JUSTIFY_RIGHT);
	...
	new FXLabel(colors,"Lumière Amb:",NULL,LAYOUT_RIGHT|LAYOUT_CENTER_Y|JUSTIFY_RIGHT);
	...
	new FXLabel(colors,"Lumière Diff:",NULL,LAYOUT_RIGHT|LAYOUT_CENTER_Y|JUSTIFY_RIGHT);
	...
	new FXLabel(colors,"Lumière Spec:",NULL,LAYOUT_RIGHT|LAYOUT_CENTER_Y|JUSTIFY_RIGHT);
	...
	new FXTabItem(panels,"Configurations\tConfigurations\tCliquer sur l'onglet Configurations.");
	...
	new FXCheckButton(settings,"Lumière",mdiclient,FXGLViewer::ID_LIGHTING,ICON_BEFORE_TEXT);
	new FXCheckButton(settings,"Brouillard",mdiclient,FXGLViewer::ID_FOG,ICON_BEFORE_TEXT);
	new FXCheckButton(settings,"Lissage",mdiclient,FXGLViewer::ID_DITHER,ICON_BEFORE_TEXT);
	new FXCheckButton(settings,"Vérouillé",mdiclient,FXGLViewer::ID_LOCK,ICON_BEFORE_TEXT);
	...
	new FXMenuTitle(menubar,"&Fichier",NULL,filemenu);
	new FXMenuCommand(filemenu,"&Nouveau...\tCtl-N",newdoc,this,ID_NOUVEAU);
	new FXMenuCommand(filemenu,"&Ouvrir...\tCtl-O",opendoc,this,ID_OPEN);
	new FXMenuCommand(filemenu,"&Enregistrer\tCtl-S",savedoc,this,ID_ENREGISTRER);
	new FXMenuCommand(filemenu,"Enregistrer &sous...\t",saveasdoc,this,ID_ENREGISTRER_SOUS);
	new FXMenuCommand(filemenu,"&Fermer\tCtl-Q\tFermer l'application.",NULL,getApp(),FXApp::ID_QUIT,MENU_DEFAULT);
    
	//Menu de création
	menuCreation=new FXMenuPane(this);
	new FXMenuTitle(menubar,"&Créer",NULL,menuCreation);
	new FXMenuCommand(menuCreation,"Surface Pol&yedrique",NULL,this,ID_CREER_SURF_POLYEDRIQUE);
	new FXMenuCommand(menuCreation,"Surface &NURBS",NULL,this,ID_CREER_SURF_NURBS);
	new FXMenuCommand(menuCreation,"Surface &Bézier",NULL,this,ID_CREER_SURF_BEZIER);
	new FXMenuCommand(menuCreation,"Surface Bézier &rationnelle",NULL,this,ID_CREER_SURF_BEZIERRAT);
	new FXMenuCommand(menuCreation,"Surface B&Spline",NULL,this,ID_CREER_SURF_BSPLINE);
	new FXMenuSeparator(menuCreation);
	new FXMenuCommand(menuCreation,"Ligne &Polygonale",NULL,this,ID_CREER_LIGNE);
	new FXMenuCommand(menuCreation,"&Segment",NULL,this,ID_CREER_SEGMENT);
	new FXMenuCommand(menuCreation,"&Polygone",NULL,this,ID_CREER_POLYGONE);
	new FXMenuSeparator(menuCreation);
	new FXMenuCommand(menuCreation,"Courbe &NURBS",NULL,this,ID_CREER_COURBE_NURBS);
	new FXMenuCommand(menuCreation,"Courbe &Bézier",NULL,this,ID_CREER_COURBE_BEZIER);
	new FXMenuCommand(menuCreation,"Courbe Bézier &rationnelle",NULL,this,ID_CREER_COURBE_BEZIERRAT);
	new FXMenuCommand(menuCreation,"Courbe B&Spline",NULL,this,ID_CREER_COURBE_BSPLINE);

	
	// Projections
	new FXFrame(toolbar,LAYOUT_TOP|LAYOUT_LEFT|LAYOUT_FIX_WIDTH|LAYOUT_FIX_HEIGHT,0,0,8,20);
	new FXButton(toolbar,"\tPerspective",new FXGIFIcon(getApp(),perspective),mdiclient,FXGLViewer::ID_PERSPECTIVE,BUTTON_AUTOGRAY|FRAME_THICK|FRAME_RAISED|LAYOUT_TOP|LAYOUT_LEFT);
	new FXButton(toolbar,"\tParallele",new FXGIFIcon(getApp(),parallel),mdiclient,FXGLViewer::ID_PARALLEL,BUTTON_AUTOGRAY|FRAME_THICK|FRAME_RAISED|LAYOUT_TOP|LAYOUT_LEFT);
	...
	new FXMenuTitle(menubar,"&Visualisation",NULL,viewmenu);
	new FXMenuCommand(viewmenu,"Parallele\t\tPour une projection parallele.",NULL,mdiclient,FXGLViewer::ID_PARALLEL,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"Perspective\t\tPour avoir une vue en perspective.",NULL,mdiclient,FXGLViewer::PERSPECTIVE,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"&Devant\t\tVue de la partie avant.",NULL,mdiclient,FXGLViewer::ID_FRONT,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"&Derrière\t\tVue de la partie arrière.",NULL,mdiclient,FXGLViewer::ID_BACK,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"&Gauche\t\tVue gauche.",NULL,mdiclient,FXGLViewer::ID_LEFT,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"&Droite\t\tVue droite.",NULL,mdiclient,FXGLViewer::ID_RIGHT,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"&Dessus\t\tVue du dessus.",NULL,mdiclient,FXGLViewer::ID_TOP,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"&Dessous\t\tVue du dessous.",NULL,mdiclient,FXGLViewer::ID_BOTTOM,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"F&it\t\tFit to view.",NULL,mdiclient,FXGLViewer::ID_FITVIEW,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"R&eset\t\tReset tous les paramètres de visualisation.",NULL,mdiclient,FXGLViewer::ID_RESETVIEW,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"Zoom...\t\tAgrandir une zone.",NULL,mdiclient,FXGLViewer::ID_LASSO_ZOOM,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"Selectionner...\t\tSelectionner une zone.",NULL,mdiclient,FXGLViewer::ID_LASSO_SELECT,MENU_AUTOGRAY);
	new FXMenuCommand(viewmenu,"Verouiller\t\tVérouiller la fenêtre de visualisation.",NULL,mdiclient,FXGLViewer::ID_LOCK,MENU_AUTOGRAY);
	
	
	// Menu d'aide
	menuAide=new FXMenuPane(this);
	new FXMenuTitle(menubar,"&Aide",NULL,menuAide,LAYOUT_RIGHT);
	new FXMenuCommand(menuAide,"&A propos de ce projet...\t",NULL,this,ID_ABOUT,0);
...
void FenetreVisualisation::create(){
	FXMainWindow::create();
	show(PLACEMENT_SCREEN);
}

//On a selectionné un nouvel objet graphique
long FenetreVisualisation::onChangedCurrentGLObject(FXObject*,FXSelector,void*)
{
	return 1;
}


/*Cette méthode permet la création d'une nouvelle ligne polygonale*/
long FenetreVisualisation::onCreerLigne(FXObject*,FXSelector,void*)
{
	/*On crée une surface par "défaut"*/
	Point3D tmp;
	ListePoints m(4);
	for (int u = 0; u < 4; u++) 
	{
		...
		tmp.y = 1.0;
		tmp.z = 1.0;
		m.modifierPoint(u,tmp);
	}
	LignePolygonale *pObjet3D = new LignePolygonale(m);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
	
	viewer->setScene(scene);
	viewer->setBounds(boite_englobante);
	return 1;
}


// A propos...
long FenetreVisualisation::onCmdAbout(FXObject*,FXSelector,void*){
	FXMessageBox::information(this,MBOX_OK,"A propos de ce projet...","Projet de POO 2001\nAuteurs :\nBLAISE Avril\nROUCH Thomas\nHEIT Cyrille");
	return 1;
}


// Ouvrir une scène
long FenetreVisualisation::onCmdOpen(FXObject*,FXSelector,void*){
	const FXchar *patterns[]={
		"Scène d'objets 3D",  "*.sc3D",NULL
	};
	FXFileDialog open(this,"Ouverture d'une scène");
	open.setPatternList(patterns);
	if(open.execute())
	{
		//On charge ici une scène d'objets 3D
		FXObject *tmp;
		FXFileStream flux;
		nom_fichier = open.getFilename().text();
		flux.open(nom_fichier,FXStreamLoad);
		flux.loadObject((FXObject *&) tmp);
		scene = (FXGLGroup *) tmp;
		FXRange boite_englobante;
		scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onEnregistrer(FXObject* a,FXSelector b,void* c){
	if(nom_fichier != "")
	{
		FXFileStream flux;
		flux.open(nom_fichier,FXStreamSave);
		flux.saveObject(scene);
	}
	else
		onEnregistrerSous(a,b,c);
	return 1;
}

//Nouvelle scène
long FenetreVisualisation::onNouveau(FXObject* a,FXSelector b,void* c){
	nom_fichier = "";
	scene->clear();
	viewer->setScene(scene);
	return 1;
...
// Enregistrer une scène dans un nouveau fichier
long FenetreVisualisation::onEnregistrerSous(FXObject*,FXSelector,void*){
	FXFileDialog savedialog(this,"Enregistrement de la Scène");
	const FXchar *patterns[]={
		"Scène d'objets 3D",  "*.sc3D",NULL
	};
	savedialog.setPatternList(patterns);
	savedialog.setFilename(nom_fichier);
	if(savedialog.execute()){
		if(FXFile::exists(savedialog.getFilename())){
			if(MBOX_CLICKED_NO==FXMessageBox::question(this,MBOX_YES_NO,"Ecrasement d'une scène","Voulez vous ecraser ce fichier : %s?",savedialog.getFilename().text())) return 1;
		}
		nom_fichier=savedialog.getFilename();
...
// Make new viewer
long FenetreVisualisation::onCmdNewViewer(FXObject*,FXSelector,void*){
	static int no=1;
...
long FenetreVisualisation::onUpdMode(FXObject* sender,FXSelector,void*){
...
*/
long FenetreVisualisation::onQueryMenu(FXObject* sender,FXSelector,void* ptr){
	FXEvent *event=(FXEvent*)ptr;
	FXMenuPane pane(this);
	new FXMenuCommand(&pane,"Parallele\t\tSwitch to parallel projection.",NULL,sender,FXGLViewer::ID_PARALLEL);
	new FXMenuCommand(&pane,"Perspective\t\tSwitch to perspective projection.",NULL,sender,FXGLViewer::ID_PERSPECTIVE);
	new FXMenuSeparator(&pane);
	new FXMenuCommand(&pane,"&Devant\t\tFront view.",NULL,sender,FXGLViewer::ID_FRONT);
	new FXMenuCommand(&pane,"&Derrière\t\tBack view.",NULL,sender,FXGLViewer::ID_BACK);
	new FXMenuCommand(&pane,"&Gauche\t\tLeft view.",NULL,sender,FXGLViewer::ID_LEFT);
	new FXMenuCommand(&pane,"&Droite\t\tRight view.",NULL,sender,FXGLViewer::ID_RIGHT);
	new FXMenuCommand(&pane,"&Dessus\t\tTop view.",NULL,sender,FXGLViewer::ID_TOP);
	new FXMenuCommand(&pane,"&Dessous\t\tBottom view.",NULL,sender,FXGLViewer::ID_BOTTOM);
	new FXMenuSeparator(&pane);
	...
	new FXMenuCommand(&pane,"Vérouillé\t\tLock view orientation",NULL,sender,FXGLViewer::ID_LOCK);
	pane.create();
...
long FenetreVisualisation::onCmdPropNurbs(FXObject *, FXSelector, void *)
{
	FenetrePropriete *x=new FenetrePropriete(this,"Propriétés des Objets 3D");
	x->execute();
	return 1;
}

/*Cette méthode permet la création d'une nouvelle surface NURBS*/
long FenetreVisualisation::onCreerSurfNURBS(FXObject*,FXSelector,void*)
{
	/*On crée une surface par "défaut"*/
	Tableau<float> knots(8,0);
	knots[0] = 0;
	knots[1] = 0;
	knots[2] = 0;
	knots[3] = 0;
	...
	for (int u = 0; u < 4; u++) {
		for (int v = 0; v < 4; v++) {
			...
			tmp.y = v;
			tmp.z = 1.0;
			m.modifierPoint(u,v,tmp);
		}
	}
	SurfaceNURBS *pObjet3D = new SurfaceNURBS(m,knots,knots);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
	
	viewer->setScene(scene);
	viewer->setBounds(boite_englobante);   /*Pour éviter un blocage*/
	return 1;
}


long FenetreVisualisation::onCreerSurfBezier(FXObject *, FXSelector, void *)
{
	/*On crée une surface par "défaut"*/
	Point3D tmp;
	MatricePoints m(4,4);
	...
	SurfaceBezier *pObjet3D = new SurfaceBezier(m);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onCreerSurfBezierRat(FXObject *, FXSelector, void *)
{
	/*On crée une surface par "défaut"*/
	Point3D tmp;
	MatricePoints m(4,4);
	...
	SurfaceBezierRat *pObjet3D = new SurfaceBezierRat(m);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onCreerSurfBSpline(FXObject *, FXSelector, void *)
{
	/*On crée une surface par "défaut"*/
	Tableau<float> knots(8,0);
	knots[0] = 0;
	...
	SurfaceBSpline *pObjet3D = new SurfaceBSpline(m,knots,knots);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onCreerSurfacePolyedrique(FXObject *, FXSelector, void *)
{
	/*On crée une surface par "défaut"*/
	Point3D tmp;
	MatricePoints m(4,4);
	...
	SurfacePolyedrique *pObjet3D = new SurfacePolyedrique(m);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onCreerSegment(FXObject *, FXSelector, void *)
{
	/*On crée un segment par défaut*/
	Point3D a(0,0,0),b(1,1,1);
	Segment *pObjet3D = new Segment(a,b);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onCreerPolygone(FXObject *, FXSelector, void *)
{
	/*On crée un polygone par défaut*/
	Point3D a(0,0,0),b(1,0,1),c(1,1,1),d(0,1,1);
	ListePoints l;
	l.ajouter(a);
	l.ajouter(b);
	l.ajouter(c);
	l.ajouter(d);
	Polygone *pObjet3D = new Polygone(l);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onSupprimerObjet3D(FXObject *a, FXSelector b, void * c)
{
	FXObject *tmp = (viewer->getSelection());
	if (tmp != NULL)
	{
		scene->remove((FXGLObject *) tmp);
		FXRange boite_englobante;
		scene->bounds(boite_englobante);  
...
/*Cette méthode permet la création d'une nouvelle courbe NURBS*/
long FenetreVisualisation::onCreerCourbeNURBS(FXObject*,FXSelector,void*)
{
	/*On crée une surface par "défaut"*/
	Tableau<float> knots(8,0);
	knots[0] = 0;
	...
	CourbeNURBS *pObjet3D = new CourbeNURBS(m,knots);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onCreerCourbeBezier(FXObject *, FXSelector, void *)
{
	/*On crée une courbe par "défaut"*/
	Point3D tmp;
	ListePoints m(4);
	for (int u = 0; u < 4; u++) 
	{
	...
	CourbeBezier *pObjet3D = new CourbeBezier(m);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onCreerCourbeBezierRat(FXObject *, FXSelector, void *)
{
	/*On crée une courbe par "défaut"*/
	Point3D tmp;
	ListePoints m(4);
	...
	CourbeBezierRat *pObjet3D = new CourbeBezierRat(m);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
...
long FenetreVisualisation::onCreerCourbeBSpline(FXObject *, FXSelector, void *)
{
	/*On crée une courbe par "défaut"*/
	Tableau<float> knots(8,0);
	knots[0] = 0;
	...
	CourbeBSpline *pObjet3D = new CourbeBSpline(m,knots);
	scene->append(pObjet3D);
	FXRange boite_englobante;
	scene->bounds(boite_englobante);  
