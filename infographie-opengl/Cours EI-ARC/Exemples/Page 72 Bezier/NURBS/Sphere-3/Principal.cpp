...
//	  gluNurbsProperty(pNurb, GLU_DISPLAY_MODE, (GLfloat)GLU_OUTLINE_POLYGON);
    ...
    int u;
    int v;
    for(u = 0, angle=0.0; u< NbPoints; u++, angle += 90.0 /*180.0/(NbPoints-1)*/)
    {
     for(v=0; v < NbPoints; v++)
     {
      PointsSurface[u][v][0] = ctrlPoints[v][0]*COS(angle); // x
      PointsSurface[u][v][1] = ctrlPoints[v][1]; // y
      PointsSurface[u][v][2] = ctrlPoints[v][0]*SIN(angle); // z
