...
// avec le Stencil On (écrire 2), sinon on voit l'ombre à travers la sphère.
  // ici on positionne la lumière sans la dessiner.
  PositionLumiere(lightPos[0], lightPos[1], lightPos[2], lightPos[3], false);

  ModeDeDessin(Mode);

  //***************************************************************
  // Limitation de l'ombre à la surface pour le sol
  if(Stencil)
  {
  ...
  //***************************************************************
  // Limitation de l'ombre à la surface pour le mur
    glDisable(GL_DEPTH_TEST);
    glColorMask(GL_FALSE, GL_FALSE, GL_FALSE, GL_FALSE);
    glEnable(GL_STENCIL_TEST);
    glStencilOp(GL_REPLACE, GL_REPLACE, GL_REPLACE);
  ...
  if(Stencil)
  {
    ...
    glPushAttrib(GL_LIGHTING_BIT);
      ...
      // Rappel :
      // Le mur est parallèle au plan XY.
      // Il est translaté de -5 unités dans la direction Z et de 5 unités
      // dans la direction Y.
      // Comme son épaisseur est de 1 unité, les coordonnées du point
      // qui se trouve sur le mur, dans le plan XZ sont : (0, 0, -4.5)
      // On obtient donc les valeur A, B, C, D de l'équation du plan :
      // Ax + By + Cz + D = 0
      //
      // A = 0
      // B = 0
      ...
      //Changement des paramètres pour dessiner sur le mur
  if(Stencil)
  {
      glEnable(GL_STENCIL_TEST);
