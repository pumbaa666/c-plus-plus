...
void evTimer(int valeur)
{
    ...
    if(valeur == 2)
    {
        cheminee += 0.07*deltaCheminee;
        if(cheminee > 1.5 || cheminee < 0.5)
        {
            if(deltaCheminee == 1.0)
                deltaCheminee = -1.0;
            else
                deltaCheminee = 1.0;
        }
        ...
        if(spinModeCheminee)
            glutTimerFunc(DELTA_TIME, evTimer, 2);
    }
}
    ...
    switch(key)
    {
      case ' ' : spinMode = !spinMode;
                 if(spinMode)
                    glutTimerFunc(DELTA_TIME, evTimer, 1);
                 break;
      case 'z' : spinModeCheminee = !spinModeCheminee;
                 if(spinModeCheminee)
                    glutTimerFunc(DELTA_TIME, evTimer, 2);
                 break;
